...
#pragma warning restore IDE0065
    ...
    public static class HandLandmarkPointBus
    {
        // 외부 프로젝트에서 구독 가능
        public static event Action<List<Transform>> OnHandResult;

        internal static void Publish(List<Transform> r) => OnHandResult?.Invoke(r);
    }
    ...
    public sealed class HandLandmarkListAnnotation : HierarchicalAnnotation
  {
    ...
    public PointAnnotation this[int index] => _landmarkListAnnotation[index];
        ...
        List<Transform> m_PalmPoint = new();
        private void Start()
        {
            _landmarkListAnnotation.Fill(_LandmarkCount);
            _connectionListAnnotation.Fill(_connections, _landmarkListAnnotation);

            var child = transform.GetComponentInChildren<PointListAnnotation>();
            if (child)
            {
                m_PalmPoint.Add(child.transform.GetChild(0));
                m_PalmPoint.Add(child.transform.GetChild(5));
                m_PalmPoint.Add(child.transform.GetChild(9));
                m_PalmPoint.Add(child.transform.GetChild(13));
                m_PalmPoint.Add(child.transform.GetChild(17));
            }
        }

        private void LateUpdate()
        {
            HandLandmarkPointBus.Publish(m_PalmPoint);
        }
